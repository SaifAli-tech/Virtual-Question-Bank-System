...
const EditMcq = () => {
  ...
  const fetchMcqData = async () => {
    try {
      ...
      setChoices(mcqData.options || ["", ""]);
      setCorrectAnswer(mcqData.answer || "");
    } catch (error) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const { _id, __v, createdAt, updatedAt, ...remainingFields } = values;
        const updatedFields = { ...remainingFields, answer: correctAnswer };
        await apiClient.put(`/mcqs/${id}`, updatedFields);
        Swal.fire({
          icon: "success",
          color: "green",
          title: "Success",
          text: `MCQ "${values.text}" updated successfully!`,
        });
        router.push("/mcqs");
      } catch (error) {
...
export default withAuth(EditMcq, ["Admin", "Teacher"]);
