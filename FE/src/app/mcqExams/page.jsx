  ...
  const fetchData = async () => {
    ...
    try {
      const response = await apiClient.get(
        `/mcqExams/pagedata?page=${page}&take=${rowsPerPage}&filter=${status}`
      );
      setExams(response.data.pagedata);
      setCount(response.data.meta.pageCount);

      router.replace(
        `/mcqExams?page=${page}&take=${rowsPerPage}&filter=${status}`
      );
    } catch (error) {
...
export default withAuth(McqExamList, ["Admin", "Teacher"]);
